        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
          ...
          >
            <h2 className="text-5xl font-black uppercase italic tracking-tighter text-slate-900">
              Moments on <span className="text-[#D4AF37]">Stage.</span>
            </h2>
          </motion.div>
          ...
          <div className="flex flex-wrap gap-4">
            {categories.map(cat => (
              <button
                ...
                ${filter === cat ? 'bg-[#0A0F1C] text-[#D4AF37] border-[#0A0F1C]' : 'bg-white text-slate-500 border-slate-200 hover:border-[#D4AF37]'}`}
              >
                {cat}
                {filter === cat && (
                  <motion.div
                    layoutId="activeFilter"
                    className="absolute inset-0 bg-[#0A0F1C] rounded-full -z-10"
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((p) => (
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#0A0F1C]/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 p-8 flex flex-col justify-end">
                  <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                    <span className="text-[#D4AF37] text-[10px] font-black uppercase tracking-widest">{p.category}</span>
                    <h3 className="text-white text-2xl font-black italic uppercase tracking-tighter">{p.title}</h3>
                    <div className="flex justify-between items-center mt-4">
                      <span className="text-white font-bold text-sm italic">{p.date}</span>
                      <button className="w-10 h-10 bg-[#D4AF37] rounded-full flex items-center justify-center text-black hover:bg-white hover:text-black transition-colors">
                        <PlayCircle size={18} />
                      </button>
                    </div>
                  </div>
